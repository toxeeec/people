...
func (s *userService) ListStatus(ctx context.Context, srcIDs []uint, userID uint) (map[uint]people.FollowStatus, error) {
	var following map[uint]struct{}
	var followed map[uint]struct{}
	g, ctx := errgroup.WithContext(ctx)
	g.Go(func() error {
		var err error
		following, err = s.fr.ListStatusFollowing(srcIDs, userID)
		return err
	})
	g.Go(func() error {
		var err error
		followed, err = s.fr.ListStatusFollowed(srcIDs, userID)
		return err
	})
	if err := g.Wait(); err != nil {
		return nil, err
	}
	fss := make(map[uint]people.FollowStatus, len(srcIDs))
...
func (s *userService) ListUsersWithStatus(ctx context.Context, srcIDs []uint, userID uint, auth bool) ([]people.User, error) {
	if len(srcIDs) == 0 {
	...
	if auth {
		g.Go(func() error {
			fss, err := s.ListStatus(context.Background(), srcIDs, userID)
			if err != nil {
				return err
			}
			select {
